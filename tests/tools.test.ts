...
import { runWithRequestHeaders } from "../src/identity.js";
...
import { PUBLIC_TOOLS, TOOL_ANNOTATIONS } from "../src/public-tools/registry.js";
...
async function call(tool: ToolDefinition, input: unknown, user = "guardian-a"): Promise<McpToolResult> {
  return await runWithRequestHeaders({ "x-user-id": user }, () => Promise.resolve(tool.handler(input)));
}
...
describe("public learning tools", () => {
  ...
  test("registry exposes exactly ten correctly annotated tools", () => {
    ...
    for (const tool of PUBLIC_TOOLS) {
      ...
      expect(TOOL_ANNOTATIONS[tool.name].openWorldHint).toBe(false);
    }
  ...
  test("profile-to-report flow is continuous, deterministic, and ID-complete", async () => {
    const created = await call(manageChildProfileTool, { action: "create", nickname: "첫째", schoolLevel: "elementary", grade: 5, interestedSubjects: ["수학"], minutesPerDay: 15 });
    ...
    expect(body(deleted).deleted).toMatchObject({ children: 1, checks: 2, plans: 1, progress: 1 });
  });

  test("user scope does not reveal another guardian's child", async () => {
    const created = await call(manageChildProfileTool, { action: "create", nickname: "둘째", schoolLevel: "middle", grade: 2 }, "guardian-a");
  ...
  test("public capability mode never lists other child IDs", async () => {
    const created = await callPublic(manageChildProfileTool, { action: "create", nickname: "공개 스코프", schoolLevel: "elementary", grade: 3 });
    const childId = body(created).childId;
    expect(body(await callPublic(manageChildProfileTool, { action: "read", childId })).profile.id).toBe(childId);
    await expect(callPublic(manageChildProfileTool, { action: "read" })).rejects.toThrow("목록을 표시하지 않습니다");
  });
