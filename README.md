...
PlayMCP 등록은 `Key/Token 인증`을 선택하고 필드명을 `x-learning-path-token`으로 설정합니다. 사용자는 비밀번호 관리자로 생성한 32자 이상의 사용자별 무작위 비밀 토큰을 한 번 연결하고 계속 재사용합니다. 서버는 이 토큰을 인증 capability로 취급해 `USER_SCOPE_SALT`와 함께 해시한 스코프에만 상태를 저장하며, 원문 토큰을 저장·반환·기록하지 않습니다. 토큰을 잃거나 바꾸면 기존 스코프를 다시 찾을 수 없고, 다른 사람에게 알려지면 해당 스코프 접근권한도 넘어가므로 안전하게 보관해야 합니다. 토큰이 없거나 32자 미만이면 교육과정 검색과 공개 학습경로 조회만 가능하며, 프로필·점검·계획·진행 기록의 저장 및 조회는 차단됩니다.
